function Game() {
	...
	this.next = function() {
		if (!p.human && p.money < 0) {
			...
			if (p.money < 0) {
				popup("<p>" + p.name + " обанкротившийся. Все его активы будут переданы " + player[p.creditor].name + ".</p>", game.bankruptcy);
			} else {
				roll();
			}
	...
	var finalizeAuction = function() {
		...
		if (highestbid > 0) { //Делаем проверку высокая цена больше 0 если True
			...
			sq.owner = highestbidder; // присваимваем недвижомость новому владельцу
			addAlert(p.name + " купил " + sq.name + " за $" + highestbid + "."); //выполняем вывод результата
		}
	...
	this.auction = function() {
		...
		popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Аукцион <span id='propertyname'></span></div><div>высокая цена= $<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, теперь ваша очередь делать ставки.</div<div><input id='bid' title='Введите сумму для участия в торгах " + s.name + ".' style='width: 291px;' /></div><div><input type='button' value='Предложение' onclick='game.auctionBid();' title='Разместите свою ставку.' /><input type='button' value='Пасс' title='Пропустите торги на этот раз.' onclick='game.auctionPass();' /><input type='button' value='Выход из Аукциона' title='Прекратить торги на " + s.name + " altogether.' onclick='if (confirm(\"Вы уверены, что хотите вообще прекратить торги на эту недвижимость?\")) game.auctionExit();' /></div>", "blank");
			...
			if (currentbidder == highestbidder) {
				...
				if (!p.human) {
					...
					if (bid === -1 || highestbid >= p.money) {
						...
						window.alert(p.name + " вышел с аукциона.");
						continue;

					} else if (bid === 0) {
						window.alert(p.name + " пассовал.");
						continue;

					} else if (bid > 0) {
						this.auctionBid(bid);
						window.alert(p.name + " предложение $" + bid + ".");
						continue;
					}
					return;
				} else {
	...
	this.auctionBid = function(bid) {
		...
		if (bid === "" || bid === null) {
			document.getElementById("bid").value = "Пожалуйста, введите ставку.";
			document.getElementById("bid").style.color = "red";
		} else if (isNaN(bid)) {
			document.getElementById("bid").value = "Ваша ставка должна быть числом.";
			document.getElementById("bid").style.color = "red";
		} else {

			if (bid > player[currentbidder].money) {
				document.getElementById("bid").value = "У вас нет достаточно денег, чтобы сделать ставку $" + bid + ".";
				document.getElementById("bid").style.color = "red";
				...
				document.getElementById("bid").value = "Ваша ставка должна быть больше самой высокой ставки. ($" + highestbid + ")";
				document.getElementById("bid").style.color = "red";
			}
		}
	};
	...
	var tradeMoneyOnKeyDown = function (e) {
		...
		// Разрешить backspace, tab, delete, клавиши со стрелками или если элемент управления был нажат, соответственно.
		if (key === 8 || key === 9 || key === 46 || (key >= 35 && key <= 40) || isCtrl) {
			return true;
		}
		...
		// Разрешить только цифровые клавиши.
		return (key >= 48 && key <= 57) || (key >= 96 && key <= 105);
	};
	...
	var tradeMoneyOnChange = function(e) {
		...
		if (isNaN(amount)) {
			this.value = "Это значение должно быть числом.";
		...
		if (amount < 0) {
			this.value = "Это значение должно быть больше, чем 0.";
			this.style.color = "red";
			return false;
		}
	...
	var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
		...
		for (var i = 0; i < 40; i++) {
			...
			// Свойство не может быть продано, если какие-либо свойства в его группе были улучшены.
			...
			if (currentSquare.owner === initiator.index) {
				...
				currentTableCellCheckbox.title = "Установите этот флажок, чтобы включить " + currentSquare.name + " в торговле.";
				...
				if (currentSquare.mortgage) {
					currentTableCell.title = "Заложенный";
		...
		if (initiator.communityChestJailCard) {
			...
			currentTableCellCheckbox.title = "Установите этот флажок, чтобы включить эту карту Get Out of Jail Free в торговлю.";
			...
			currentTableCell.textContent = "Выйти из тюрьмы бесплатно карты";
		...
		if (initiatorSideTable.lastChild) {
			...
			initiatorProperty.textContent = initiator.name + " не имеет свойств для торговли.";
		}

		if (recipientSideTable.lastChild) {
			recipientProperty.appendChild(recipientSideTable);
		} else {
			recipientProperty.textContent = recipient.name + " не имеет свойств для торговли.";
		}
		...
		if (allowRecipientToBeChanged && pcount > 2) {
			...
			nameSelect.title = "Выберите игрока для торговли.";
		} else {
	...
	this.acceptTrade = function(tradeObj) {
		if (isNaN(document.getElementById("trade-leftp-money").value)) {
			document.getElementById("trade-leftp-money").value = "Это значение должно быть числом.";
			document.getElementById("trade-leftp-money").style.color = "red";
			return false;
		}

		if (isNaN(document.getElementById("trade-rightp-money").value)) {
			document.getElementById("trade-rightp-money").value = "Это значение должно быть числом.";
		...
		if (money > 0 && money > initiator.money) {
			document.getElementById("trade-leftp-money").value = initiator.name + " не обладающий $" + money + ".";
			document.getElementById("trade-leftp-money").style.color = "red";
			return false;
		} else if (money < 0 && -money > recipient.money) {
			document.getElementById("trade-rightp-money").value = recipient.name + " не обладающий $" + (-money) + ".";
			document.getElementById("trade-rightp-money").style.color = "red";
			return false;
		}
		...
		if (isAPropertySelected === 0) {
			popup("<p>Для торговли необходимо выбрать одно или несколько свойств.</p>");

			return false;
		}

		if (showAlerts && !confirm(initiator.name + ", вы уверены, что хотите сделать этот обмен с" + recipient.name + "?")) {
			return false;
		}
		...
		for (var i = 0; i < 40; i++) {
			...
			if (tradeObj.getProperty(i) === 1) {
				...
				addAlert(recipient.name + " полученный " + square[i].name + " от " + initiator.name + ".");
			} else if (tradeObj.getProperty(i) === -1) {
				square[i].owner = initiator.index;
				addAlert(initiator.name + " полученный " + square[i].name + " от " + recipient.name + ".");
			}
		...
		if (tradeObj.getCommunityChestJailCard() === 1) {
			...
			addAlert(recipient.name + ' получил карточку "выйти из тюрьмы бесплатно" от ' + initiator.name + ".");
		} else if (tradeObj.getCommunityChestJailCard() === -1) {
			initiator.communityChestJailCard = true;
			recipient.communityChestJailCard = false;
			addAlert(initiator.name + ' получил карточку "выйти из тюрьмы бесплатно" от ' + recipient.name + ".");
		}
		...
		if (tradeObj.getChanceJailCard() === 1) {
			...
			addAlert(initiator.name + ' rполучил карточку "выйти из тюрьмы бесплатно" от ' + recipient.name + ".");
		}
		...
		if (money > 0) {
			...
			addAlert(recipient.name + " полученный $" + money + " от " + initiator.name + ".");
		} else if (money < 0) {
			money = -money;
			...
			addAlert(initiator.name + " полученный $" + money + " от " + recipient.name + ".");
		}
	...
	this.proposeTrade = function() {
		...
		if (isNaN(document.getElementById("trade-rightp-money").value)) {
			document.getElementById("trade-rightp-money").value = "Это значение должно быть числомr.";
			document.getElementById("trade-rightp-money").style.color = "red";
			return false;
		}
		...
		if (initiator.human && !confirm(initiator.name + ", вы уверены, что хотите сделать это предложение " + recipient.name + "?")) {
			return false;
		}
		...
		if (recipient.human) {
			...
			addAlert(initiator.name + " инициировал торговлю с " + recipient.name + ".");
			popup("<p>" + initiator.name + " предложил сделку с вами, " + recipient.name + ". Вы можете принять, отклонить или изменить предложение.</p>");
		} else {
			var tradeResponse = recipient.AI.acceptTrade(tradeObj);

			if (tradeResponse === true) {
				popup("<p>" + recipient.name + " принял ваше предложение.</p>");
				this.acceptTrade(reversedTrade);
			} else if (tradeResponse === false) {
				popup("<p>" + recipient.name + " отклонил ваше предложение.</p>");
				return;
			} else if (tradeResponse instanceof Trade) {
				popup("<p>" + recipient.name + " предложил встречное предложение.</p>");
	...
	this.eliminatePlayer = function() {
		...
		if (pcount === 1) {
			...
			popup("<p>Поздравления, " + player[1].name + ", вы выиграли игру.</p><div>");
	...
	this.bankruptcyUnmortgage = function() {
		...
		var HTML = "<p>" + player[p.creditor].name + ", вы можете отменить перезакладывание любого из следующих свойств, без процентов, нажав на них. Нажмите кнопку ОК, когда закончите.</p><table>";
		var price;
	...
	this.resign = function() {
		popup("<p>Вы уверены, что хотите уйти в отставку?</p>", game.bankruptcy, "Yes/No");
	};
	...
	this.bankruptcy = function() {
		...
		addAlert(p.name + " обанкротившийся.");
		...
		if (pcount === 2 || bankruptcyUnmortgageFee === 0 || p.creditor === 0) {
			...
			addAlert(pcredit.name + " проплаченный $" + bankruptcyUnmortgageFee + " проценты на заложенное имущество, полученные от " + p.name + ".");
			popup("<p>" + pcredit.name + ", вы должны заплатить $" + bankruptcyUnmortgageFee + " проценты по заложенной недвижимости, которые вы получили от " + p.name + ".</p>", function() {player[pcredit.index].pay(bankruptcyUnmortgageFee, 0); game.bankruptcyUnmortgage();});
		}
	};
...
function updateOwned() {
	...
	for (var i = 0; i < 40; i++) {
		...
		if (sq.owner == turn) {
			...
			mortgagetext = "";
			if (sq.mortgage) {
				mortgagetext = "title='Заложенный' style='color: grey;'";
			}

			housetext = "";
			if (sq.house >= 1 && sq.house <= 4) {
				for (var x = 1; x <= sq.house; x++) {
					housetext += "<img src='images/house.png' alt='' title='Дом' class='house' />";
				}
			} else if (sq.hotel) {
				housetext += "<img src='images/hotel.png' alt='' title='Отель' class='hotel' />";
			}
	...
	if (HTML === "") {
		HTML = p.name + ", у вас нет никаких свойств.";
		$("#option").hide();
	} else {
		HTML += "</table>";
	}
...
function updateOption() {
	...
	if (checkedproperty < 0 || checkedproperty >= 40) {
		...
		$("#buildings").show();
		document.getElementById("buildings").innerHTML = "<img src='images/house.png' alt='' title='Дом' class='house' />:&nbsp;" + housesum + "&nbsp;&nbsp;<img src='images/hotel.png' alt='' title='Отель' class='hotel' />:&nbsp;" + hotelsum;
	...
	if (sq.mortgage) {
		...
		if (sq.groupNumber >= 3) {
			...
			buyhousebutton.value = "Покупать дома ($" + sq.houseprice + ")";
			sellhousebutton.value = "Продам дом ($" + (sq.houseprice * 0.5) + ")";
			buyhousebutton.title = "Купить дом для $" + sq.houseprice;
			sellhousebutton.title = "Продам дом за $" + (sq.houseprice * 0.5);

			if (sq.house == 4) {
				buyhousebutton.value = "Купить отель ($" + sq.houseprice + ")";
				buyhousebutton.title = "Купить отель для $" + sq.houseprice;
			}
			if (sq.hotel == 1) {
				$("#buyhousebutton").hide();
				sellhousebutton.value = "Продам отель ($" + (sq.houseprice * 0.5) + ")";
				sellhousebutton.title = "Продам гостиницу за $" + (sq.houseprice * 0.5);
			}
			...
			for (var i = 0; i < max; i++) {
				...
				if (s.owner !== sq.owner) {
					...
					buyhousebutton.title = "Прежде чем вы сможете купить дом, вы должны владеть всеми свойствами этой цветовой группы.";
				} else {
			...
			if (sq.house > minhouse) {
				...
				if (sq.house == 1) {
					buyhousebutton.title = "Прежде чем вы сможете купить другой дом, другие свойства этой цветовой группы должны иметь один дом.";
				} else if (sq.house == 4) {
					buyhousebutton.title = "Прежде чем вы сможете купить отель, другие свойства этой цветовой группы должны иметь 4 дома.";
				} else {
					buyhousebutton.title = "Прежде чем вы сможете купить дом, все остальные свойства этой цветовой группы должны иметь " + sq.house + " дома.";
				}
			}
			if (sq.house < maxhouse) {
				...
				if (sq.house == 1) {
					sellhousebutton.title = "Прежде чем вы сможете продать дом, другие свойства этой цветовой группы должны иметь один дом.";
				} else {
					sellhousebutton.title = "Прежде чем вы сможете продать дом, все остальные свойства этой цветовой группы должны иметь " + sq.house + " дома.";
				}
			}
			...
			if (!allGroupUninproved) {
				document.getElementById("mortgagebutton").title = "Прежде чем имущество может быть заложено, все свойства его цветовой группы должны быть улучшены.";
...
function addamount(amount, cause) {
	...
	addAlert(p.name + " полученный $" + amount + " от " + cause + ".");
}
...
function subtractamount(amount, cause) {
	...
	addAlert(p.name + " потерянный $" + amount + " от " + cause + ".");
}

function gotojail() {
	var p = player[turn];
	addAlert(p.name + " был отправлен прямо в тюрьму.");
	document.getElementById("landed").innerHTML = "Вы находитесь в тюрьме.";

	p.jail = true;
	doublecount = 0;

	document.getElementById("nextbutton").value = "Конец поворота";
	document.getElementById("nextbutton").title = "Конец хода и перейти к следующему игроку.";
...
function payeachplayer(amount, cause) {
	...
	addAlert(p.name + " потерянный $" + total + " от " + cause + ".");
}
...
function collectfromeachplayer(amount, cause) {
	...
	addAlert(p.name + " полученный $" + total + " от " + cause + ".");
}
...
function advance(destination, pass) {
	...
	if (typeof pass === "number") {
		if (p.position < pass) {
			...
			p.position = pass;
			p.money += 200;
			addAlert(p.name + " собрал $ 200 зарплату за прохождение НАЧАТЬ.");
		}
	}
...
function streetrepairs(houseprice, hotelprice) {
	...
	if (cost > 0) {
		...
		if (houseprice === 40) {
			addAlert(p.name + " потерянный $" + cost + " Сундук сообщества.");
		} else {
			addAlert(p.name + " потерянный $" + cost + " Шанс.");
		}
	}
...
function payfifty() {
	...
	addAlert(p.name + " заплатил штраф в 50 долларов, чтобы выйти из тюрьмы.");
	updateMoney();
	updatePosition();
}
...
function useJailCard() {
	...
	addAlert(p.name + " использовал \"Выходите из тюрьмы бесплатно\" карту.");
	updateOwned();
	updatePosition();
}
	...
	if (p.money - sq.houseprice < 0) {
		...
		if (sq.house < 4) {
			if (houseSum >= 32) {
				...
				sq.house++;
				addAlert(p.name + " разместил дом на " + sq.name + ".");
			}
			...
			if (hotelSum >= 12) {
				...
				sq.house = 5;
				sq.hotel = 1;
				addAlert(p.name + " размещен отель на" + sq.name + ".");
			}
		}
...
function sellHouse(index) {
	...
	if (sq.hotel === 1) {
		sq.hotel = 0;
		sq.house = 4;
		addAlert(p.name + " продается отель на " + sq.name + ".");
	} else {
		sq.house--;
		addAlert(p.name + " продается дом на " + sq.name + ".");
	}
...
function showStats() {
	...
	for (var x = 1; x <= pcount; x++) {
		...
		if (!write) {
			HTML += p.name + " не имеет никаких свойств.";
		} else {
			HTML += "</table>";
		}
...
function buy() {
	...
	if (p.money >= cost) {
		...
		property.owner = turn;
		updateMoney();
		addAlert(p.name + " купили " + property.name + " для " + property.pricetext + ".");
		...
		popup("<p>" + p.name + ", тебе нужно $" + (property.price - p.money) + " еще купить " + property.name + ".</p>");
	}
}
...
function land(increasedRent) {
	...
	if (s.owner !== 0 && s.owner != turn && !s.mortgage) {
		...
		addAlert(p.name + " проплаченный $" + rent + " сдается в аренду " + player[s.owner].name + ".");
		p.pay(rent, s.owner);
		player[s.owner].money += rent;

		document.getElementById("landed").innerHTML = "Вы приземлились на " + s.name + ". " + player[s.owner].name + " собранный $" + rent + " rent.";
	} else if (s.owner > 0 && s.owner != turn && s.mortgage) {
		document.getElementById("landed").innerHTML = "Вы приземлились на " + s.name + ".Имущество заложено, арендная плата не взималась.";
	}
	...
	if (p.position === 30) {
		...
		if (p.human) {
			popup("<div>Отправиться в тюрьму. Идите прямо в тюрьму. Не проходите мимо идите. Не собирают $200.</div>", gotojail);
		} else {
			gotojail();
		}
...
function roll() {
	...
	document.getElementById("nextbutton").title = "Завершите поворот и перейдите к следующему игроку.";
	...
	if (die1 == die2) {
		addAlert(p.name + " прокат " + (die1 + die2) + " - удваивает.");
	} else {
		addAlert(p.name + " прокат " + (die1 + die2) + ".");
	}
	...
	if (die1 == die2 && !p.jail) {
		...
		if (doublecount < 3) {
			document.getElementById("nextbutton").value = "Кинуть еще раз";
			document.getElementById("nextbutton").title = "Вы бросили удваивается. Кинуть еще раз.";

		// If player rolls doubles three times in a row, send him to jail
		} else if (doublecount === 3) {
			p.jail = true;
			doublecount = 0;
			addAlert(p.name + " двойные три раза подряд.");
			updateMoney();


			if (p.human) {
				popup("Ты три раза подряд бросал дубли. Отправиться в тюрьму.", gotojail);
			} else {
				gotojail();
			}
	...
	if (p.jail === true) {
		...
		if (die1 == die2) {
			...
			addAlert(p.name + " кинул двойники, чтобы выйти из тюрьмы.");
			...
			if (p.jailroll === 3) {
				...
				if (p.human) {
					popup("<p>Вы должны заплатить штраф в размере $50.</p>", function() {
						payFifty();
						payfifty();
...
function play() {
	...
	document.getElementById("nextbutton").value = "Бросьте Кости";
	document.getElementById("nextbutton").title = "Бросьте кости и переместите свой жетон соответственно.";
	...
	if (p.jail) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "Вы находитесь в тюрьме.<input type='button' title='Заплатите $ 50 штрафа, чтобы выйти из тюрьмы немедленно.' value='Заплатить 50 долларов штрафа' onclick='payfifty();' />";
		...
		document.getElementById("nextbutton").title = "Бросить кости. Если вы бросите двойников, вы выйдете из тюрьмы.";
		...
		if (p.jailroll === 0)
			...
			document.getElementById("landed").innerHTML += "<div>Примечание: Если вы не бросаете двойников после этого броска, вы <i>должены</i> заплатите штраф в размере $ 50.</div>";
			addAlert("Это" + p.name + "'третья очередь в тюрьму.");
		}
...
window.onload = function() {
	...
	$("#mortgagebutton").click(function() {
		...
		if (s.mortgage) {
			if (player[s.owner].money < Math.round(s.price * 0.6)) {
				popup("<p>Тебе нужно$" + (Math.round(s.price * 0.6) - player[s.owner].money) + " больше  unmortgage " + s.name + ".</p>");

			} else {
				popup("<p>" + player[s.owner].name + ", вы уверены, что хотите unmortgage " + s.name + " для $" + Math.round(s.price * 0.6) + "?</p>", function() {
					unmortgage(checkedProperty);
				}, "Yes/No");
			}
		} else {
			popup("<p>" + player[s.owner].name + ", вы уверены, что хотите заложить " + s.name + " для $" + Math.round(s.price * 0.5) + "?</p>", function() {
	...
	$("#buyhousebutton").on("click", function() {
		...
		if (p.money < s.houseprice) {
			if (s.house === 4) {
				popup("<p>Тебе нужно $" + (s.houseprice - player[s.owner].money) + " купить отель для " + s.name + ".</p>");
				return;
			} else {
				popup("<p>Тебе нужно$" + (s.houseprice - player[s.owner].money) + " больше чтобы купить дом для " + s.name + ".</p>");
				return;
			}
		}
		...
		if (s.house < 4 && houseSum >= 32) {
			popup("<p>Aвсего в собственности 32 дома. Вы должны ждать, пока он станет доступен.</p>");
			return;
		} else if (s.house === 4 && hotelSum >= 12) {
			popup("<p>Все 12 отелей находятся в собственности. Вы должны ждать, пока он станет доступен.</p>");
			return;
		}
